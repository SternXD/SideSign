...
NSString *SIDECertificatePEMPrefix = @"-----BEGIN CERTIFICATE-----";
NSString *SIDECertificatePEMSuffix = @"-----END CERTIFICATE-----";

@implementation SIDECertificate
...
- (nullable instancetype)initWithData:(NSData *)data
{
    ...
    NSData *prefixData = [data subdataWithRange:NSMakeRange(0, MIN(data.length, SIDECertificatePEMPrefix.length))];
    NSString *prefix = [[NSString alloc] initWithData:prefixData encoding:NSUTF8StringEncoding];
    
    if (![prefix isEqualToString:SIDECertificatePEMPrefix])
    {
        ...
        NSString *content = [NSString stringWithFormat:@"%@\n%@\n%@", SIDECertificatePEMPrefix, base64Data, SIDECertificatePEMSuffix];
...
{
    SIDECertificate *certificate = (SIDECertificate *)object;
    if (![certificate isKindOfClass:[SIDECertificate class]])
    {
        return NO;
    }
