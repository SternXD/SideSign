...
#import "SIDEAppID.h"
#import "SIDETeam.h"
#import "SIDECertificate.h"
#import "SIDEProvisioningProfile.h"
#import "SIDEApplication.h"
...
std::string CertificatesContent(SIDECertificate *altCertificate)
{
...
- (NSProgress *)signAppAtURL:(NSURL *)appURL provisioningProfiles:(NSArray<SIDEProvisioningProfile *> *)profiles completionHandler:(void (^)(BOOL success, NSError *error))completionHandler
{    
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        SIDEProvisioningProfile *(^profileForApp)(SIDEApplication *) = ^SIDEProvisioningProfile *(SIDEApplication *app) {
            // Assume for now that apps don't have 100s of app extensions 🤷‍♂️
            for (SIDEProvisioningProfile *profile in profiles)
            {
        ...
        NSError * (^prepareApp)(SIDEApplication *) = ^NSError *(SIDEApplication *app) {
            ...
            for (NSString *entitlement in profile.entitlements)
            {
                ...
                {
                    if ([entitlement isEqualToString:SIDEEntitlementKeychainAccessGroups])
                    {
                ...
                {
                    if ([entitlement isEqualToString:SIDEEntitlementApplicationIdentifier] || [entitlement isEqualToString:SIDEEntitlementTeamIdentifier] || [entitlement isEqualToString:SIDEEntitlementGetTaskAllow])
                    {
        ...
        for (SIDEApplication *appExtension in application.appExtensions)
        {   
