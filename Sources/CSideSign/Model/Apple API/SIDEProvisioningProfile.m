...
@implementation SIDEProvisioningProfile
...
- (nullable instancetype)initWithData:(NSData *)data
{
    ...
    if (self)
    {
        NSDictionary *dictionary = [SIDEProvisioningProfile dictionaryFromEncodedData:data];
        if (dictionary == nil)
        {
            return nil;
        }
        ...
        NSDictionary<SIDEEntitlement, id> *entitlements = dictionary[@"Entitlements"];
        ...
        [entitlements enumerateKeysAndObjectsUsingBlock:^(SIDEEntitlement entitlement, id value, BOOL *stop) {
            if (![entitlement isEqualToString:SIDEEntitlementApplicationIdentifier])
            {
                return;
            }
        ...
        NSMutableArray<SIDECertificate *> *certificates = [NSMutableArray array];
        
        NSArray *certificatesArray = dictionary[@"DeveloperCertificates"];
        for (NSData *data in certificatesArray)
        {
            SIDECertificate *certificate = [[SIDECertificate alloc] initWithData:data];
            if (certificate != nil)
            {
...
{
    SIDEProvisioningProfile *profile = [[SIDEProvisioningProfile alloc] initWithData:self.data];
...
{
    SIDEProvisioningProfile *profile = (SIDEProvisioningProfile *)object;
    if (![profile isKindOfClass:[SIDEProvisioningProfile class]])
    {
        return NO;
    }
