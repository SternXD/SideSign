...
int SIDEReadBufferSize = 8192;
int SIDEMaxFilenameLength = 512;
char SIDEDirectoryDeliminator = '/';
...
- (BOOL)unzipArchiveAtURL:(NSURL *)archiveURL toDirectory:(NSURL *)directoryURL progress:(NSProgress *)progress error:(NSError **)error
{
    ...
    FILE *outputFile = nil;
    char buffer[SIDEReadBufferSize];
    ...
    {
        unz_file_info info;
        char cFilename[SIDEMaxFilenameLength];
        
        if (unzGetCurrentFileInfo(zipFile, &info, cFilename, SIDEMaxFilenameLength, NULL, 0, NULL, 0) != UNZ_OK)
        {
        ...
        if (![filename hasPrefix:@"__MACOSX"] && [filename characterAtIndex:filename.length - 1] != SIDEDirectoryDeliminator)
        {
    ...
    for (int i = 0; i < zipInfo.number_entry; i++)
    {
        ...
        if ([filename characterAtIndex:filename.length - 1] == SIDEDirectoryDeliminator)
        {
            // Directory
        ...
        else
        {
            ...
            do
            {
                result = unzReadCurrentFile(zipFile, buffer, SIDEReadBufferSize);
