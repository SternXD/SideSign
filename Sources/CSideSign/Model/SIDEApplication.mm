...
SIDEDeviceType SIDEDeviceTypeFromUIDeviceFamily(NSInteger deviceFamily)
{
    switch (deviceFamily)
    {
        case 1: return SIDEDeviceTypeiPhone;
        case 2: return SIDEDeviceTypeiPad;
        case 3: return SIDEDeviceTypeAppleTV;
        default: return SIDEDeviceTypeNone;
    }
}
...
@implementation SIDEApplication
...
- (instancetype)initWithFileURL:(NSURL *)fileURL
{
    ...
    if (self)
    {
        ...
        SIDEDeviceType supportedDeviceTypes = SIDEDeviceTypeNone;
        
        if ([deviceFamilies isKindOfClass:[NSNumber class]])
        {
            NSInteger rawDeviceFamily = [(NSNumber *)deviceFamilies integerValue];
            supportedDeviceTypes = SIDEDeviceTypeFromUIDeviceFamily(rawDeviceFamily);
        }
        ...
        {
            supportedDeviceTypes = SIDEDeviceTypeiPhone;
        }
...
- (NSDictionary<SIDEEntitlement,id> *)entitlements
{
...
- (SIDEProvisioningProfile *)provisioningProfile
{
    if (_provisioningProfile == nil)
    {
        ...
        _provisioningProfile = [[SIDEProvisioningProfile alloc] initWithURL:provisioningProfileURL];
    }
    
    return _provisioningProfile;
}

- (NSSet<SIDEApplication *> *)appExtensions
{
    ...
    for (NSURL *fileURL in enumerator)
    {
        ...
        SIDEApplication *appExtension = [[SIDEApplication alloc] initWithFileURL:fileURL];
        if (appExtension == nil)
        {
            continue;
        }
