...
public extension SIDEAppleAPI {
    @objc func authenticate(appleID unsanitizedAppleID: String,
                            password: String,
                            anisetteData: SIDEAnisetteData,
                            verificationHandler: ((@escaping (String?) -> Void) -> Void)?,
                            completionHandler: @escaping (SIDEAccount?, SIDEAppleAPISession?, Error?) -> Void) {
        ...
        do {
            ...
            guard let publicKey = context.start() else { throw SIDEAppleAPIError(.authenticationHandshakeFailed) }
            ...
            sendAuthenticationRequest(parameters: parameters, anisetteData: anisetteData) { result in
                do {
                    ...
                    guard let verificationMessage = context.makeVerificationMessage(iterations: iterations, isHexadecimal: isHexadecimal) else {
                        throw SIDEAppleAPIError(.authenticationHandshakeFailed)
                    }
                    ...
                    self.sendAuthenticationRequest(parameters: parameters, anisetteData: anisetteData) { result in
                        do {
                            ...
                            guard context.verifyServerVerificationMessage(serverVerificationMessage) else { throw SIDEAppleAPIError(.authenticationHandshakeFailed) }
                            guard let decryptedData = serverDictionary.decryptedCBC(context: context) else { throw SIDEAppleAPIError(.authenticationHandshakeFailed) }
                            ...
                            case "trustedDeviceSecondaryAuth":
                                guard let verificationHandler = verificationHandler else { throw SIDEAppleAPIError(.requiresTwoFactorAuthentication) }
                            ...
                            default:
                                ...
                                guard let checksum = context.makeChecksum(appName: app) else { throw SIDEAppleAPIError(.authenticationHandshakeFailed) }
                                ...
                                self.fetchAuthToken(app: app, parameters: parameters, context: context, anisetteData: anisetteData) { result in
                                    ...
                                    case let .success(token):
                                        ...
                                        let session = SIDEAppleAPISession(dsid: dsid, authToken: token, anisetteData: anisetteData)
...
private extension SIDEAppleAPI {
    func fetchAuthToken(app: String, parameters: [String: Any], context: GSAContext, anisetteData: SIDEAnisetteData, completionHandler: @escaping (Result<String, Error>) -> Void) {
        sendAuthenticationRequest(parameters: parameters, anisetteData: anisetteData) { result in
            do {
                let responseDictionary = try result.get()

                guard let encryptedToken = responseDictionary["et"] as? Data else { throw URLError(.badServerResponse) }
                guard let token = encryptedToken.decryptedGCM(context: context) else { throw SIDEAppleAPIError(.authenticationHandshakeFailed) }
        ...
        let requestCodeTask = session.dataTask(with: request) { data, _, error in
            do {
                ...
                func responseHandler(verificationCode: String?) {
                    do {
                        guard let verificationCode = verificationCode else { throw SIDEAppleAPIError(.requiresTwoFactorAuthentication) }
                        ...
                        let verifyCodeTask = self.session.dataTask(with: request) { data, _, error in
                            do {
                                guard let data = data else { throw error ?? SIDEAppleAPIError(.unknown) }
                                ...
                                switch errorCode {
                                case -21669: throw SIDEAppleAPIError(.incorrectVerificationCode)
                                default:
                                    guard let errorDescription = responseDictionary["em"] as? String else { throw SIDEAppleAPIError(.unknown) }

                                    let localizedDescription = errorDescription + " (\(errorCode))"
                                    throw SIDEAppleAPIError(.unknown, userInfo: [NSLocalizedDescriptionKey: localizedDescription])
                                }
                            } catch {
                                completionHandler(.failure(error))
                            }
                    ...
                    do {
                        ...
                        let verifyCodeTask = self.session.dataTask(with: request) { _, response, error in
                            do {
                                ...
                                else { throw SIDEAppleAPIError(.incorrectVerificationCode) }
    ...
    func fetchAccount(session: SIDEAppleAPISession, completionHandler: @escaping (Result<SIDEAccount, Error>) -> Void) {
        ...
        sendRequest(with: url, additionalParameters: nil, session: session, team: nil) { responseDictionary, requestError in
            do {
                guard let responseDictionary = responseDictionary else { throw requestError ?? SIDEAppleAPIError(.unknown) }
                ...
                guard let account = try self.processResponse(responseDictionary, parseHandler: { () -> Any? in
                    ...
                    let account = SIDEAccount(responseDictionary: dictionary)
                    return account
                }, resultCodeHandler: nil) as? SIDEAccount else {
                    throw SIDEAppleAPIError(.unknown)
                }
...
private extension SIDEAppleAPI {
    func sendAuthenticationRequest(parameters requestParameters: [String: Any], anisetteData: SIDEAnisetteData, completionHandler: @escaping (Result<[String: Any], Error>) -> Void) {
        do {
            ...
            let dataTask = session.dataTask(with: request) { data, _, error in
                do {
                    ...
                    switch errorCode {
                    case -20101, -22406: throw SIDEAppleAPIError(.incorrectCredentials)
                    case -22421: throw SIDEAppleAPIError(.invalidAnisetteData)
                    default:
                        guard let errorDescription = status["em"] as? String else { throw SIDEAppleAPIError(.unknown) }
    ...
    func makeTwoFactorCodeRequest(url: URL,
                                  dsid: String,
                                  idmsToken: String,
                                  anisetteData: SIDEAnisetteData) -> URLRequest {
